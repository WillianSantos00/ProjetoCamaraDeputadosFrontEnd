...
import { useSearchParams } from "react-router-dom"; // Importa o hook para ler parâmetros da URL
...
function Home() {
  ...
  // --- Estados dos Filtros ---
  const [valorInput, setValorInput] = useState(initialSearchTerm);
  ...
  const deputadosFiltrados = useMemo(() => {
    ...
    return dados.filter(dado => {
      if (!dado || !dado.id || !dado.nome) return false;
      if (valorInput && !dado.nome.toLowerCase().includes(valorInput.toLowerCase())) return false;
      if (valorPartido && (!dado.siglaPartido || !dado.siglaPartido.toLowerCase().includes(valorPartido.toLowerCase()))) return false;
      if (valorEstado && (!dado.siglaUf || !dado.siglaUf.toLowerCase().includes(valorEstado.toLowerCase()))) return false;
      if (genero && (!dado.sexo || dado.sexo !== genero)) return false;
      if (situacao) {
        const situacaoDeputado = dado.ultimoStatus?.situacao;
        if (!situacaoDeputado || situacaoDeputado.toLowerCase() !== situacao.toLowerCase()) return false;
      }
      return true;
    });
  }, [valorInput, valorPartido, valorEstado, genero, situacao]);
  ...
  const handleGeneroChange = (e) => { setGenero(e.target.value); setCurrentPage(1); };
  const handleSituacaoChange = (e) => { setSituacao(e.target.value); setCurrentPage(1); };
